...
export class NewServicesComponent implements OnInit {
 ...
 ngOnInit(): void {
  ...
  // Define antes de tudo
  this.priceTypes = [
    { value: 'EXACT', label: 'Fixo' },
    { value: 'STARTING_FROM', label: 'A partir de' }
  ];

  // Tenta recuperar service via state ou query param
  ...
  // carrega tipos de empresa e marcas
  ...
  this.brandService.getAllBrands(this.authService.getCompanyVehicle()).subscribe({
    next: (data) => {
      this.brandList = data;
      this.filteredBrandList = this.brandList.slice(0, 5);
    },
    error: () => {}
  });
}
  ...
  private mapServiceToUI(): void {
    ...
    // Adiciona marcas que não estão nos modelos
  ...
  searchModelByBrand(): void {
    if (this.brandSelected) {
      this.modelService.getModelByBrand(this.brandSelected.idBrand).subscribe({
        next: (data) => {
          this.modelList = data;
          this.filteredModelList = this.modelList ? this.modelList.slice(0, 5) : [];
        },
        error: () => {}
      });
    }
  ...
  filterModels(): void {
    if (!this.searchModelTerm?.trim()) {
      this.filteredModelList = this.modelList.slice(0, 5);
      return;
    }
  ...
  trackByBrandModel(index: number, item: brandModelSelected): string {
    return `${item.brand?.idBrand}-${item.model?.idModel || 'noModel'}`;
  }
  createRequestObject() {
